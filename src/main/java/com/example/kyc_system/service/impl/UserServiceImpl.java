...
public class UserServiceImpl implements UserService {
    ...
    public UserDTO createUser(UserDTO userDTO) {
        ...
        // Fallback for registration flow where filter is excluded
        if (tenantId == null) {
            tenantId = userDTO.getTenantId();
        }

        if (tenantId == null || tenantId.isBlank()) {
            throw new RuntimeException("Tenant ID is required for user creation");
        }
    ...
    public void deleteUser(Long id) {
        ...
        if (TenantContext.isSuperAdmin()) {
            user = userRepository.findById(id).orElseThrow(() -> new RuntimeException("User not found: " + id));
        } else {
            user = userRepository.findByIdAndTenantId(id, currentTenant).orElseThrow(() -> new RuntimeException("User not found in your tenant scope: " + id));
        }

        // Use the user's actual tenant for document cleanup
        String userTenantId = user.getTenantId();
        kycRequestRepository.findByUserIdAndTenantId(id, userTenantId).forEach(request ->
            request.getKycDocuments().forEach(kycDocumentService::deleteDocument)
        );

        userRepository.deleteById(id);
        log.info("User deleted: id={}, email={}, tenantId={} (by Super Admin: {})",
            id, user.getEmail(), userTenantId, TenantContext.isSuperAdmin());
    }
    ...
    public String login(LoginDTO loginDTO) {
        ...
        return jwtTokenProvider.generateToken(authentication, user.getTenantId(), user.getId());
    }
