...
public class TenantController {
    ...
    public ResponseEntity<TenantStatsDTO> getStats(@PathVariable String tenantId) {
        checkTenantAccess(tenantId);
    ...
    private void checkTenantAccess(String tenantId) {
        ...
        if (!isSuperAdmin) {
            String userTenantId = com.example.kyc_system.context.TenantContext.getTenant();
            if (userTenantId == null || !userTenantId.equals(tenantId)) {
                throw new com.example.kyc_system.exception.BusinessException("Access denied: You can only access your own tenant statistics.");
            }
        }
    }
}