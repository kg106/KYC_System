...
class RefreshTokenServiceImplTest {
    ...
    void processRefreshToken_Valid_ReturnsNewTokens() {
        ...
        when(userRepository.findByEmail("user@test.com")).thenReturn(Optional.of(User.builder().id(1L).tenantId("default").userRoles(new java.util.HashSet<>()).build()));
        when(jwtTokenProvider.generateToken(eq("user@test.com"), anyString(), anyLong(), anyList())).thenReturn("new-access-token");
